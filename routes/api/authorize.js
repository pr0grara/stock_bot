...
const { MFA, authenticateToken, validateToken } = require('../../authorize_util');
const route = express.Router();

route.post('/validate-token', async (req, res) => {
    let token = req.body.token;
    let validated = await validateToken(token);
    if (!validated) return res.status(200).send(false).end();
    if (validated) return res.status(200).send(true).end();
...
route.post('/mfa-attempt', cors(true), async (req, res) => {
    ...
    return res.status(401).send(false).end();
});
...
route.post('/new-mongo-chart-token', cors(true), async (req, res) => {
    ...
    if (!validToken) return res.status(404).send('unauthorized').end();
